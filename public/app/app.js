            ...
            {urls.map((u, i) => {
              ...
              return (
                <div className="profile-url-row" key={i}>
                  ...
                  {hasUrl && (
                    <a className="profile-url-link" href={hrefUrl} target="_blank" rel="noreferrer" title={hrefUrl}>
                      {displayUrl}
                    </a>
                  )}
...
/* ============================================================
   BOOKMARKS
   ============================================================ */
function Bookmarks({ bookmarks, onAdd, onDelete, onEdit }) {
          ...
          <div className="calendar-week" key={wi}>
            {week.map((dateKey, di) => {
              ...
              return (
                <div
                  key={di}
                  className={`calendar-cell${!dateKey ? ' empty' : ''}${isToday ? ' today' : ''}${isInSelecting(dateKey) ? ' selecting' : ''}`}
                ...
                >
                  {dateKey && <div className="calendar-cell-daynum">{parseInt(dateKey.slice(-2), 10)}</div>}
                  {dateKey && (
...
function App() {
  ...
  const THEMES = ['pink', 'blue', 'mint', 'lavender', 'peach'];
  ...
  const CARDS = {
    ...
    bookmarks: {
      title: '🔖 Bookmarks',
      render: () => <Bookmarks bookmarks={bookmarks} onAdd={handleAddBookmark} onDelete={handleDeleteBookmark} onEdit={handleEditBookmark} />,
    },
    memo: {
      title: '💬 Memo Feed',

...
/* ============================================================
   PROJECT PROGRESS
   ============================================================ */
function ProjectProgress({ stages, checked, onToggle, onOpenSettings }) {
  ...
  return (
    <div className="progress-container">
      <div className="progress-toprow">
        <div className="progress-percent">{pct}%</div>
        <button className="btn-icon" onClick={onOpenSettings} title="진행 단계 설정" aria-label="진행 단계 설정">⚙</button>
      </div>
      <div className="progress-bar-track">
        <div className="progress-bar-fill" style={{ width: `${pct}%`, background: `hsl(${hue}, 72%, 55%)` }} />
      </div>
      {total === 0 ? (
        <div className="empty-state">설정(⚙)에서 진행 단계를 추가해주세요. 예: 아이디어, 초고, 퇴고, 업로드</div>
      ) : (
        <div className="progress-stage-list">
          {stages.map((s, i) => (
            <label className="progress-stage-item" key={i}>
              <span className={checked[i] ? 'progress-stage-label done' : 'progress-stage-label'}>{s}</span>
...
function ProgressSettingsModal({ stagesText, onSave, onClose }) {
  const [text, setText] = useState(stagesText);
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        <div className="modal-titlebar">
          ...
          <div className="modal-title-text">📈 진행 단계 설정</div>
        </div>
        <div className="modal-body">
          <div className="form-group">
            <span className="form-label">단계 (쉼표로 구분, 순서대로)</span>
            <input type="text" className="form-input" value={text} onChange={e => setText(e.target.value)}
              placeholder="예: 아이디어, 초고, 퇴고, 업로드" autoFocus />
            <span className="form-hint">쉼표(,)로 구분해서 입력하면 순서대로 체크박스가 만들어져요. 여기서 단계를 수정·추가·삭제할 수 있어요.</span>
          </div>
          <div className="modal-actions">
            <button className="btn-secondary" onClick={onClose}>취소</button>
            <button className="btn-primary" onClick={handleSave}>저장</button>
...
function CalendarEventPopover({ initial, onSave, onDelete, onClose }) {
  ...
  function handleSave() {
    ...
    onSave({ ...initial, title: t, start: s, end: e, color, importance, isDeadline });
  }
...
function Calendar({ events, onAdd, onEdit, onDelete }) {
    ...
    function onUp() {
      setSelecting(cur => {
        if (cur) {
          ...
          setPopover({ title: '', start: s, end: e, color: 0, importance: 1, isDeadline: false });
        }
        return null;
      });
    }
  ...
  function handleSavePopover(data) {
    if (data.id) onEdit(data); else onAdd({ ...data, id: uid() });
    setPopover(null);
  }
...
function App() {
  ...
  const [progressStagesText, setProgressStagesText] = useState(() => loadLS('wt_progress_stages_text', '아이디어,초고,퇴고,업로드'));
  const [progressChecked, setProgressChecked] = useState(() => loadLS('wt_progress_checked', []));
  ...
  useEffect(() => { saveLS('wt_progress_stages_text', progressStagesText); }, [progressStagesText]);
  useEffect(() => { saveLS('wt_progress_checked', progressChecked); }, [progressChecked]);
  ...
  const progressStages = progressStagesText.split(',').map(s => s.trim()).filter(Boolean);
  function handleToggleProgress(i) {
    setProgressChecked(prev => { const next = [...prev]; next[i] = !next[i]; return next; });
  }
  function handleSaveProgressStages(text) {
    const stages = text.split(',').map(s => s.trim()).filter(Boolean);
    setProgressStagesText(text);
    setProgressChecked(prev => stages.map((_, i) => !!prev[i]));
  }
  ...
  const CARDS = {
    ...
    calendar: {
      title: '📅 Calendar',
      render: () => <Calendar events={calendarEvents} onAdd={handleAddEvent} onEdit={handleEditEvent} onDelete={handleDeleteEvent} />,
    },
    progress: {
      title: '📈 Progress',
      render: () => <ProjectProgress stages={progressStages} checked={progressChecked} onToggle={handleToggleProgress} onOpenSettings={() => setProgressModalOpen(true)} />,
    },
  };
    ...
    <div className="app-window">
      ...
      {progressModalOpen && (
        <ProgressSettingsModal
          stagesText={progressStagesText}
          onSave={handleSaveProgressStages}
